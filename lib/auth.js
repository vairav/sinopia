...
Auth.prototype.base_url = function base_url(req){
  var self = this;
  var base = self.config.url_prefix ? self.config.url_prefix.replace(/\/$/, '')
    : req.protocol + '://' + req.get('host')
  return base;
};

Auth.prototype.set_session = function (req, res, login, password) {
  var self = this;
  var str = login + ':' + password;
  res.cookies.set('token', self.aes_encrypt(str).toString('base64'));
  res.redirect(self.base_url(req));
};
